...
def post_url(args):
    ...
    try:
        ...
        logging.debug(f"Timeout Error : {errd}")
        logging.debug(f"URLs : {args[0]}")
        logging.debug(f"ERRD: {type(errd)}")
        return errd

    except requests.exceptions.ConnectionError as errc:
        logging.debug(f"Error Connecting {errc}")
        logging.debug(f"URLs : {args[0]}")
        logging.debug(f"ERRC:{type(errc)}")
        return errc

    except requests.exceptions.HTTPError as errb:
        logging.debug(f"Http Error : {errb}")
        logging.debug(f"URLs : {args[0]}")
        logging.debug(f"ERRB: {type(errb)}")
        return errb

    # Any Error except upper exception
    except requests.exceptions.RequestException as erra:
        logging.debug(f"AnyException : {erra}")
        logging.debug(f"URLs : {args[0]}")
        logging.debug(f"ERRA: {type(erra)}")
        return erra
...
class FederatedDiscoveryEngine:
    ...
    def generate_combinations(self, input_string):
        result = []
        if len(input_string)==1:
            ...
            while input_string.count(' ') >=0:
                if input_string.count(' ')== 0:
                        ...
                        break
                result.append(input_string.split())
                input_string=input_string.replace(' ', '_', 1)
                
        return result

    def make_query(self, combi, dic):
        result = []
        for c in combi:
            keys = [dic.get(item) for item in c]
            if None in keys:
                continue
    ...
    def federated_search_ctrl(self, datahubs, query, body={}):
        ...
        query_combi= self.generate_combinations(query)
        intent_query = self.make_query(query_combi, integrated_intents)
