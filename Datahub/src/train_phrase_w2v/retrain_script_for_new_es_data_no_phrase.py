...
es = Elasticsearch(config1['elasticsearch'])
...
def get_data_with_date(start_date='1970-01-01', end_date=datetime.now().strftime('%Y-%m-%d')):
    ...
    result= es.search(index=config1['hub_index_name'], body=query_body, size=5)
    # 검색 결과 있는지 확인
    while len(result['hits']["hits"]) != 0:
        ...
        result=es.search(index=config1['hub_index_name'], body= new_query_body)
...
def train_model(datasets, model_path='ko_new.bin', binary=True):
    ...
    print('new_model.bin' + "'s train success!!")
