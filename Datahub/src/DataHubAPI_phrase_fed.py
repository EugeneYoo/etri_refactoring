...
from classes.pagination_multi import Pagination
...
logging.debug(config['hub_index_name'])
# api를 사용할 객체 선언
...
# query를 통해 사용자 의도 분석을 수행하고 점수를 포함한 결과를 반환, phrase를 위한 통합 검색 로직 적용
...
def analyze_intent_with_score():
    ...
    logging.debug("query: ", query, "aid:", algorithm_id)
    ...
    if algorithm_id == '1':
        ...
        threshold = float(body['parameter'][0]['similarity'])
        # 명사구 체크
        phrase = hub.check_query_phrase(query)

        # 찾은 명사구에 대한 분석을 위해 형태에 맞게 변환함
        phrase_string = " ".join(phrase)
        intents = hub.analyze_intent_with_score(phrase_string, threshold)
...
def apply_facet_filter():
    ...
    logging.debug(f"데이터셋 크기: {len(datasets)}")
    logging.debug(f"데이터셋 카운트 크기: {dataset_count}")
