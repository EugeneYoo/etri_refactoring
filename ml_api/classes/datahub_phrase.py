...
class DataHub:
    ...
    def __init__(self, **kwargs):
        ...
        # print(kwargs['verify_certs'], verify_certs)
    ...
    def search(self, query_list):
        ...
        # 검색 결과 받아오는 사이즈 크기
        size = 10000
        print(f"hhh:{self.hub_index_name}")
        result = self.es.search(index=self.hub_index_name, body=result_body, size=size)
       
        search_result = []
        #result = test_result
        print(f"spend time using es python api: {result['took']} , {result['hits']['total']['value']}")
    ...
    def get_search_aggs(self, query, algorithm_id="", algorithm_parameter=0.5):
        ...
        result = self.es.search(index=self.hub_index_name, body=result_body, size=0)
    ...
    def check_query_phrase(self, query):
        ...
        while(i < len(queryList)):
            ...
            for j in range(i + 1, len(queryList)):
                ...
                phrase_test = self.analyze_intent(checkword)
                print(f'TTTTTTT:::::{phrase_test})')
        ...
        for index in range(len(resultList)):
            if "_" in resultList[index]:
                resultList2.append(resultList[index])
            else:
                resultList2.extend(Mecab.nouns(resultList[index]))
