from .modelConfig import modelDir, datasetDir, params
...
class Ranker:
    ...
    def __init__(self):
        ...
        self.qds = QDS()
        self.qds.load_state_dict(torch.load(modelDir["qds"]))
        self.qds.eval()

        try:
            self.qdsRawDataset = pd.read_csv(datasetDir["qdsRawDataset"], encoding='utf-8-sig', index_col=False)
        except:
            self.qdsRawDataset = None
        self.doc2vec = Doc2Vec.load(modelDir["doc2vec"])
        self.doc2vec.random.seed(9999)
        self.resultDatasetList = []
        self.queryVector = []
        self.clickedDatasetVector = []
        self.mode_modified_time= int(os.path.getmtime(modelDir["qds"]))
        self.reloadQdsModel()
    ...
    def _score(self, query, datasetList):
        ...
        for elem in datasetList:
            ...
            try:
                ...
                #print("scoring!!!")
                #print(f"qds score: {score} |||| {score.item()}")
    ...
    def rank(self, datasetList, query):
        ...
        '''
        print("in rank function")
        scoreList, datasetList = self._score(query, datasetList)
        #qds Score 높은 순으로 정렬
        scoreList = sorted(scoreList, key=lambda document: (document["qdsScore"] + document["elasticScore"] * params["scoreWeight"]), reverse=True)
    ...
    """
    def _getDataDict(self, datasetList):
        '''
        :param datasetList: keyword search 결과 데이터셋 리스트
        :return: {id : datasetInfo} 형태의 딕셔너리
        '''
        for elem in datasetList:
            self.dataDict[elem.id] = elem
        #{1 : {"id" : 1, "title" : "버스 데이터"}}
